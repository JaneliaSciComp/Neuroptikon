...
for neuron in display.network.neurons():
    ...
    somaY += (len(cluster) - 1) / 2.0 * 0.011 - cluster.index(neuron.name) * 0.011
    display.setVisiblePosition(neuron, (somaX, somaY, 0.0))
...
for synapse in display.network.synapses():
    ...
    display.setVisibleWeight(synapse, 0.5 if synapse.getAttribute('Count').value < 5 else 1.0)

# Make all gap junctions gray and weight them according to their connection count.
for gapJunction in display.network.gapJunctions():
    display.setVisibleColor(gapJunction, (0.0, 0.0, 0.0))
    display.setVisibleWeight(gapJunction, 0.5 if gapJunction.getAttribute('Count').value < 5 else 1.0)
